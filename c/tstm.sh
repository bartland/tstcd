...
[[ "$platform" =~ "MINGW*" ]] && {
    gcc -Wl,--whole-archive tst_objects.a -Wl,--no-whole-archive -Wl,--export-all-symbols -shared -o libmytst.dll -Wl,--out-implib,libmytst.dll.a -Wl,--major-image-version,3,--minor-image-version,0 $LINK_FLAGS; }
[[ "$platform" == "Darwin" ]] && {
...
[[ "$platform" =~ "MINGW*" ]] && {
    gcc -Wl,--whole-archive tstx_objects.a -Wl,--no-whole-archive -o mytstx.exe -Wl,--major-image-version,3,--minor-image-version,0  libmytst.dll.a $LINK_FLAGS; }
[[ "$platform" == "Darwin" ]] && {
